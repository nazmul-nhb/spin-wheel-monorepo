...
function runSpins(): void {
	...
	for (let i = 0; i < count; i++) {
		...
		log(logEl, `Spin #${i + 1}: ${result.segment.label} (index=${result.index}, angle=${result.finalAngle.toFixed(1)}°)`);
	...
	for (const [label, winCount] of sorted) {
		...
		lines.push(`  ${label.padEnd(12)} ${String(winCount).padStart(3)}× (${pct.padStart(5)}%) ${bar}`);
	}
